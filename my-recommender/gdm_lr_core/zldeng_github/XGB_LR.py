...
class XGBoostLR(object):
    ...
    def __init__(self, xgb_model_name, lr_model_name,
                 ...
                 xgb_eval_metric='mlogloss', xgb_n_jobs=1, n_estimators=100):
    ...
    def trainModel(self, train_x, train_y):
        ...
        sys.stdout.flush()
        self.xgb_clf = xgb.XGBClassifier(n_jobs=self.xgb_n_jobs)
        ...
        with open(self.xgb_model_name, 'wb') as f:
            pickle.dump(self.xgb_clf, f, True)
        with open(self.lr_model_name, 'wb') as f:
            pickle.dump(self.lr_clf, f, True)
        with open(self.one_hot_encoder_model_name, 'wb') as f:
            pickle.dump(self.one_hot_encoder, f, True)

        print('Train xgboost and lr model done')

    def loadModel(self):
        with open(self.xgb_model_name, 'rb') as f:
            self.xgb_clf = pickle.load(f)
        with open(self.lr_model_name, 'rb') as f:
            self.lr_clf = pickle.load(f)
        with open(self.one_hot_encoder_model_name, 'rb') as f:
